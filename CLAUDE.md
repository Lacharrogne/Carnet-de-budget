# Instructions de travail — Carnet de budget 🪙
...
- **Aucun dialogue natif.** Zéro `window.confirm` / `prompt` / `alert` :
  utiliser `ConfirmActionModal` (`src/components/ui/ConfirmActionModal.tsx`).
...
- **Brouillons de formulaires** : hook `useFormDraft` (`localStorage`, 12 h),
  mini-formulaires compris.
- **Chiffres d'argent** : toute logique de calcul doit rester testable et pure.
